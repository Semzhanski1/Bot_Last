...
def dz1(bot, chat_id):
    name = "Liza"
    bot.send_message(chat_id, text=f"Приветствую,меня зовут Лиза!")

def dz2(bot,chat_id):
    my_inputInt(bot,chat_id,"Сколько лет назад мама родила?",dz2_ResponseHandler)

def dz2_ResponseHandler(bot, chat_id, age_int):
    if age_int < 0:
        bot.send_message(chat_id, text=f"0! тебе всего {age_int}!\n кажется ты ещё не родился..."
                                        f"Старость не радоть.")
    elif age_int > 100:
        bot.send_message(chat_id, text=f"{age_int}! \n Вам не сюда надо , а в книгу рекордов.")
    elif age_int > 7:
        bot.send_message(chat_id, text=f"{age_int}! \n Вы ещё молодой!.")
    else:
        bot.send_message(chat_id, text=f"Какой твой любимый мультик?")
...
def dz4(bot, chat_id):
    dz4_ResponseHandler = lambda message: bot.send_message(chat_id, text=f"Я владею твои именем, {message.text[0:][0:] }!")
